...
public class Ch_08_Proj_1 {
  ...
  public static void main(String[] args) throws IOException {
    int value;
    Ch_08_Proj_1 app = new Ch_08_Proj_1();
    ...
    // loop through array joining into larger tree with '+' as root
    for (int i = 1; i < str.length() ; i++) {
      Node temp = app.new Node('+');
      temp.leftChild = trees[i-1].root;
      temp.rightChild = trees[i].root;
      Tree tempTree = app.new Tree();
      tempTree.root = temp;
      trees[i] = tempTree;
    }

    trees[str.length() - 1].displayTree();
